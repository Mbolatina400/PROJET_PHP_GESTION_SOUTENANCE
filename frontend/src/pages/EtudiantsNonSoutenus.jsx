...
function EtudiantsNonSoutenus() {
  ...
  return <div className="non-soutenus-page">
    ...
    <label className="single-search student-search-input"><span aria-hidden="true">⌕</span><input type="search" aria-label="Rechercher un étudiant non soutenu" placeholder="Rechercher par nom ou matricule..." value={recherche} onChange={(e) => setRecherche(e.target.value)} /></label>
    <section className="non-soutenus-list-section"><div className="section-heading"><div><p className="section-kicker">Suivi des soutenances</p><h3>Étudiants à programmer</h3></div><p><strong>{etudiantsFiltres.length}</strong> étudiant{etudiantsFiltres.length > 1 ? "s" : ""}</p></div>
      {loading ? <p className="loading-state">Chargement des étudiants…</p> : <table><colgroup><col style={{ width: "16%" }} /><col style={{ width: "23%" }} /><col style={{ width: "27%" }} /><col style={{ width: "16%" }} /><col style={{ width: "18%" }} /></colgroup><thead><tr><th>Matricule</th><th>Nom</th><th>Prénoms</th><th>Niveau</th><th>Parcours</th></tr></thead><tbody>{etudiantsFiltres.length === 0 ? <tr><td colSpan="5" className="student-no-result">{recherche ? "Aucun étudiant ne correspond à votre recherche." : "Tous les étudiants ont soutenu."}</td></tr> : etudiantsFiltres.map((e) => <tr key={e.matricule}><td>{e.matricule}</td><td>{e.nom}</td><td>{e.prenoms}</td><td><span className={`level-badge level-${String(e.niveau).toLowerCase()}`}>{e.niveau}</span></td><td><span className={`parcours-badge parcours-${String(e.parcours).toLowerCase()}`}>{e.parcours}</span></td></tr>)}</tbody></table>}
    </section>
  </div>;
}
